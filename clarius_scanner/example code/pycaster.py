...
def main():
    ...
    parser.add_argument(
        "--address", "-a", dest="ip", help="ip address of probe.", required=False
    )
    parser.add_argument(
        "--port", "-p", dest="port", type=int, help="port of the probe", required=False
    )
    parser.add_argument(
        "--width", "-w", dest="width", type=int, help="image output width in pixels"
    )
    parser.add_argument(
        "--height", "-ht", dest="height", type=int, help="image output height in pixels"
    )
    parser.set_defaults(ip="192.168.1.1")
    parser.set_defaults(port=32997)
    ...
    while key != "q" and key != "Q":
        ...
        if key == "a" or key == "A":
            key = input(
                "(f)->freeze (i)->capture image (c)->capture cine, (d/D)->depth, (g/G)->gain: "
            )
            if key == "f" or key == "F":
