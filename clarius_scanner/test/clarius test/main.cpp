...
#ifdef _MSC_VER
    fopen_s(&fp, "C:\\Users\\codyh\\Downloads\\raw_data.tar", "wb+");
#else
    fp = fopen("C:\\Users\\codyh\\Downloads", "wb+");
#endif
    if (!fp)
    {
        cleanup();
        PRINT << "failed to open" << std::endl;

        return false;
    }
    else {
        PRINT << "opened" << std::endl;
    }
