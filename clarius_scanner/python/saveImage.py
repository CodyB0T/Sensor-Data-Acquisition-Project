...
def newRawImage(image, lines, samples, bps, axial, lateral, timestamp, jpg, rf, angle):
    ...
    if rf == 1 and onlyone == True and jpg == 0:
        ...
        # arr = np.frombuffer(image, dtype=np.float16)
        # # arr.shape == (14400,)

        # print(arr)

        # df = pd.DataFrame(arr)
        # df.to_csv("float16.csv")

        # img.save("raw_image.jpg")
        # df = pd.DataFrame(list(img.getdata()))
        # df.to_csv(f"testData_{lines}x{samples}.csv", index=False)
        # # print(f"saved {lines}, {samples}")
        # print(f"{image} next ")
        # print(f"{list(image)} next")
        # df = pd.DataFrame(list(image))
        # onlyone = False
