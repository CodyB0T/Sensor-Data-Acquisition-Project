...
if len(sys.argv) < 2:
    ...
    if len(sys.argv) == 2:
        ...
        print(
            "Current sampling rate: %d (%.2fHz)"
            % (samplingrate[0], (32768.0 / samplingrate[0]))
        )
    elif len(sys.argv) == 3:
        ...
        if 1 <= newrate <= 65535:
            ...
            # send get sampling rate command
            ser.write(
                struct.pack("BBB", 0x05, (newrate & 0xFF), ((newrate & 0xFF00) >> 8))
            )  # need to be done this way due to alignment issues
            wait_for_ack()
            print("Sampling rate set to: %d (%.2fHz)" % (newrate, (32768.0 / newrate)))
        else:
            print(
                "%d is not a valid sampling rate\nMust be between 1 and 65535" % newrate
            )
    # close serial port
    ser.close()
    print("")
